...
export default function AiChat() {
  ...
  // ✨ THE ULTIMATE BRAIN MERGE: SUPABASE + GEMINI ✨
  const processAIResponse = async (userText: string) => {
    const text = userText.toLowerCase();
    let foundProduct = null;
    let isActionable = false;

    // 1. SUPABASE PRE-SEARCH (Find the product first)
    ...
    try {
      ...
      // We feed Gemini the rules of CARTIO + whatever product Supabase just found!
      ...
      Knowledge Base:
      - Shipping: Complimentary nationwide over Rs. 2500. Otherwise Rs. 250.
      - Payment: Cash on Delivery (COD) and Credit/Debit Cards via 256-Bit SSL.
      - Returns: 7-day Satisfaction Ledger (returns allowed within 7 days).
      - Tracking: Users can track orders via the Tracking Node in the main menu.
      - Human Agent: Users can connect to a human via the WhatsApp button on the bottom left.
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9990] flex flex-col items-end">
      ...
      {/* --- THE GEMINI AI WINDOW --- */}
      <div 
        className={`mb-4 w-[calc(100vw-3rem)] sm:w-[420px] bg-zinc-950/95 backdrop-blur-3xl border border-white/10 rounded-3xl shadow-[0_0_100px_rgba(139,92,246,0.3)] overflow-hidden transition-all duration-500 origin-bottom-right flex flex-col ${isOpen ? 'scale-100 opacity-100 h-[600px] max-h-[85vh] pointer-events-auto' : 'scale-50 opacity-0 h-0 pointer-events-none'}`}
      >
        ...
        <div className="p-5 md:p-6 border-b border-white/10 flex items-center justify-between bg-black/40 shrink-0 mt-1">
          ...
          <button onClick={() => setIsOpen(false)} className="w-10 h-10 rounded-full bg-white/5 text-white hover:bg-white hover:text-black transition-all flex items-center justify-center group">
                ...
                <div className="mt-3 w-[85%] bg-black border border-white/10 rounded-2xl overflow-hidden shadow-xl">
                  <div className="flex gap-3 p-3">
                    <div className="w-16 h-16 bg-zinc-900 rounded-xl overflow-hidden shrink-0 border border-white/5 relative">
                      <img src={msg.product.image} alt={msg.product.name} className="absolute inset-0 w-full h-full object-contain p-1 opacity-90" />
                    </div>
                  ...
                  {msg.product.stock === 0 && (
                    <div className="w-full py-2 bg-zinc-900 text-zinc-500 text-[10px] font-black uppercase tracking-[0.3em] text-center border-t border-white/5">
                      Out of Stock
                    </div>
                  )}
                </div>
        ...
        {/* ✨ QUICK ACTIONS UI (Scrollable Horizontal List) ✨ */}
        <div className="bg-black/60 backdrop-blur-md pt-4 pb-2 px-5 flex gap-2 overflow-x-auto custom-scrollbar border-t border-white/5 shrink-0 relative z-20">
      ...
      <button 
        ...
        className={`flex items-center justify-center w-[60px] h-[60px] rounded-full shadow-[0_0_25px_rgba(147,51,234,0.4)] hover:scale-110 hover:shadow-[0_0_35px_rgba(147,51,234,0.6)] transition-all duration-500 relative group ${isOpen ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : 'bg-white text-black'}`}
      ...
      >
        {isOpen ? (
          ...
          <>
            ...
            {/* Spinning Star inside the button */}
            <svg className="w-8 h-8 relative z-10 text-black group-hover:text-purple-600 transition-colors animate-[spin_6s_linear_infinite]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2L13.5 8.5L20 10L13.5 11.5L12 18L10.5 11.5L4 10L10.5 8.5L12 2Z"/></svg>
            
            <span className="absolute right-[75px] bg-zinc-900 text-white text-xs font-bold px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none shadow-xl border border-white/10">
              Consult Gemini
            </span>
          </>
        )}
